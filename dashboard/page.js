  ...
  useEffect(() => {
    if (user && typeof window !== 'undefined') {
      ...
      if (paymentSuccess === 'true') {
        ...
        toast.success('🎉 Payment successful! Your material is ready to download.')
  ...
  const fetchDownloadHistory = async () => {
    try {
      ...
      const { data: downloads, error } = await supabase
        ...
        .select('id, material_id, material_title, material_type, downloaded_at')
        .eq('user_id', user.id)
        .order('downloaded_at', { ascending: false })

      if (error) throw error

      // Then get material details for each download
      const downloadsWithMaterials = await Promise.all(
        (downloads || []).map(async (download) => {
          const { data: material } = await supabase
            .from('materials')
            .select('id, title, description, subject, class, thumbnail_url, pdf_url')
      ...
      setDownloadHistory(downloadsWithMaterials)
    } catch (error) {
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                      ...
                      <div className="flex flex-col sm:flex-row gap-3 lg:flex-col lg:items-end">
                        <div className="text-right">
                          ...
                          <p className="text-xs sm:text-sm text-gray-500">
                            Transaction: {purchase.paymentId?.slice(-8) || 'N/A'}
                          </p>
                        </div>
