...
export async function POST(request) {
  try {
    const { razorpay_order_id, razorpay_payment_id, razorpay_signature, materialId, couponCode } = await request.json()
    ...
    if (supabaseAdmin) {
      ...
      // Track influencer order if coupon was used
      if (couponCode || payment.coupon_code) {
        try {
          const usedCoupon = couponCode || payment.coupon_code
          await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/influencer/track-order`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              couponCode: usedCoupon,
              orderAmount: payment.original_amount || payment.amount,
          ...
          console.error('Error tracking influencer order:', trackError)
          // Don't fail the payment for this
        }
      }
